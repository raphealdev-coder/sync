...
export default function CategoryLinkerPage() {
  const { stores, selectedStoreId, setSelectedStoreId } = useStores();
  const [eposCategories, setEposCategories] = useState<EposCategory[]>([]);
  ...
  const [selectedEpos, setSelectedEpos] = useState<EposCategory | null>(null);
  const [selectedWoo, setSelectedWoo] = useState<WooCategory | null>(null);
  ...
  const fetchEpos = async () => {
    ...
    try {
      ...
      setError(`Failed to load ePOS categories: ${err}`);
    } finally {
  ...
  const fetchWoo = async () => {
    ...
    setLoadingWoo(true);
    try {
      const res = await fetch(`/api/woo/categories?store_id=${selectedStoreId}`);
      const data = await res.json();
      if (data.error) throw new Error(data.error);
      setWooCategories(data.categories ?? []);
    } catch (err) {
      setError(`Failed to load WooCommerce categories: ${err}`);
    } finally {
      setLoadingWoo(false);
    }
  };
  ...
  const fetchMappings = async () => {
    if (!selectedStoreId) return;
    try {
      const res = await fetch(`/api/category-mappings?store_id=${selectedStoreId}`);
      const data = await res.json();
  ...
  const handleLink = async () => {
    ...
    try {
      ...
      setSuccessMsg(`Linked "${selectedEpos.Name}" (ePOS) → "${selectedWoo.name}" (Woo)`);
  ...
  const handleUnlink = async (mappingId: number) => {
    try {
      setError('');
      const res = await fetch(`/api/category-mappings?id=${mappingId}`, { method: 'DELETE' });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? 'Failed');
      setSuccessMsg('Category link removed.');
      fetchMappings();
    } catch (err) {
      setError(String(err));
    }
    ...
    <div className="p-8 max-w-6xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Link ePOS Now categories to WooCommerce categories</p>
        </div>
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-100 mb-6">
        <h2 className="font-semibold text-slate-700 mb-4">Link Categories</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* ePOS side */}
          <div>
            <div className="flex items-center gap-2 mb-3">
              <h3 className="text-sm font-medium text-slate-600">ePOS Now Category</h3>
            ...
            {selectedEpos ? (
              <div className="flex items-center justify-between bg-indigo-50 border border-indigo-200 rounded-lg px-4 py-3">
                <div>
                  <p className="font-medium text-sm text-indigo-700">{selectedEpos.Name}</p>
                  <p className="text-xs text-indigo-500">ID: {selectedEpos.Id}</p>
                </div>
              ...
              <>
                <div className="relative mb-2">
                  ...
                  <input type="text" placeholder="Search ePOS categories..." value={searchEpos} onChange={(e) => setSearchEpos(e.target.value)} className="w-full pl-9 pr-3 py-2 border border-slate-200 rounded-lg text-sm" />
                </div>
                <div className="max-h-48 overflow-y-auto border border-slate-100 rounded-lg divide-y divide-slate-50">
                  {filteredEpos.map((c) => (
                    <button key={c.Id} onClick={() => setSelectedEpos(c)} disabled={mappedEposIds.has(String(c.Id))} className={`w-full text-left px-3 py-2 text-sm hover:bg-slate-50 ${mappedEposIds.has(String(c.Id)) ? 'opacity-40' : ''}`}>
                      ...
                      <span className="text-slate-400 ml-2 text-xs">ID: {c.Id}</span>
                    </button>
                  ))}
                  {filteredEpos.length === 0 && <p className="text-sm text-slate-400 p-3">No categories found</p>}
                </div>
              </>
            )}
          </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              <h3 className="text-sm font-medium text-slate-600">WooCommerce Category</h3>
              {loadingWoo && <RefreshCw className="w-3 h-3 animate-spin text-slate-400" />}
            </div>
            {selectedWoo ? (
              <div className="flex items-center justify-between bg-indigo-50 border border-indigo-200 rounded-lg px-4 py-3">
                <div>
                  <p className="font-medium text-sm text-indigo-700">{selectedWoo.name}</p>
                  <p className="text-xs text-indigo-500">ID: {selectedWoo.id} {selectedWoo.count !== undefined && `(${selectedWoo.count} products)`}</p>
                </div>
              ...
              <>
                <div className="relative mb-2">
                  <Search className="w-4 h-4 absolute left-3 top-2.5 text-slate-400" />
                  <input type="text" placeholder="Search WooCommerce categories..." value={searchWoo} onChange={(e) => setSearchWoo(e.target.value)} className="w-full pl-9 pr-3 py-2 border border-slate-200 rounded-lg text-sm" />
                </div>
                <div className="max-h-48 overflow-y-auto border border-slate-100 rounded-lg divide-y divide-slate-50">
                  {filteredWoo.map((c) => (
                    <button key={c.id} onClick={() => setSelectedWoo(c)} disabled={mappedWooIds.has(c.id)} className={`w-full text-left px-3 py-2 text-sm hover:bg-slate-50 ${mappedWooIds.has(c.id) ? 'opacity-40' : ''}`}>
                      <span className="font-medium">{c.name}</span>
                      <span className="text-slate-400 ml-2 text-xs">{c.count !== undefined ? `${c.count} products` : `ID: ${c.id}`}</span>
                    </button>
                  ))}
                  {filteredWoo.length === 0 && <p className="text-sm text-slate-400 p-3">No categories found</p>}
                </div>
              </>
            )}
          </div>
        ...
        <div className="mt-4 flex justify-center">
          <button onClick={handleLink} disabled={!selectedEpos || !selectedWoo || linking} className="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 text-white font-medium px-5 py-2.5 rounded-lg text-sm transition-colors">
            ...
            {linking ? 'Linking…' : 'Link Selected Categories'}
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100">
          <h2 className="font-semibold text-slate-700">Linked Categories ({mappings.length})</h2>
        </div>
        {mappings.length === 0 ? (
          <p className="text-sm text-slate-400 p-6">No category links yet. Use the form above to link categories.</p>
        ) : (
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-slate-500 text-xs uppercase">
              <tr>
                <th className="text-left px-6 py-3">ePOS Category</th>
                <th className="text-left px-6 py-3">WooCommerce Category</th>
